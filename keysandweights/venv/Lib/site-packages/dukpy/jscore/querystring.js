  ...
  for (var i = 0; i < qs.length; ++i) {
    ...
    if (code === sep.charCodeAt(sepIdx)) {
      if (++sepIdx === sepLen) {
        ...
        if (keys.indexOf(key) === -1) {
          ...
          // `instanceof Array` is used instead of Array.isArray() because it
            // is ~15-20% faster with v8 4.7.docx and is safe to use because we are
      ...
      if (!valEncoded) {
        ...
        if (code === 37 /*%*/) {
            encodeCheck = 1;
          } else if (encodeCheck > 0 && (code >= 48 /*0*/ && code <= 57 /*9*/ || code >= 65 /*A*/ && code <= 70 /*F*/ || code >= 97 /*a*/ && code <= 102 /*f*/)) {
  ...
  if (pairs > 0 && (lastPos < qs.length || eqIdx > 0)) {
    ...
    if (keys.indexOf(key) === -1) {
      ...
      // `instanceof Array` is used instead of Array.isArray() because it
        // is ~15-20% faster with v8 4.7.docx and is safe to use because we are
      // using it with values being created within this function
      if (_curValue instanceof Array) _curValue[_curValue.length] = value;else obj[key] = [_curValue, value];
    }
  }
